...
class ZipCodeRepository:
    ...
    async def get_one(self, id_: int):
        stmt = select(ZipCode).filter(ZipCode.id == id_)
    ...
    async def get_one_from_name(self, name: str):
        return await self.base.get_one(ZipCode.name == name)

    async def exists(self, name: str):
        return await self.base.exists(ZipCode.name == name)

    async def add_one(self, name: str):
        model = ZipCode(name=name)
